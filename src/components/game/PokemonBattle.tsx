...
export function PokemonBattle({
  ...
  const handlePlayerMove = (moveIndex: number) => {
    ...
    setTimeout(() => {
      ...
      let effText = "";
      if (
        (move.type === "Elétrico" && currentOpponent.type.includes("Água")) ||
        (move.type === "Fogo" && currentOpponent.type.includes("Planta")) ||
        (move.type === "Planta" && currentOpponent.type.includes("Terra")) ||
        (move.type === "Lutador" &&
          (currentOpponent.type.includes("Normal") || currentOpponent.type.includes("Aço")))
      ) {
        mult = 1.6;
        effText = " Foi super efetivo!";
      } else if (
        (move.type === "Elétrico" && currentOpponent.type.includes("Terra")) ||
        (move.type === "Fogo" && currentOpponent.type.includes("Água")) ||
        (move.type === "Planta" && currentOpponent.type.includes("Fogo"))
      ) {
        mult = 0.6;
        effText = " Não foi muito efetivo...";
      }

      const variance = 0.85 + Math.random() * 0.3;
      const damage = Math.max(8, Math.round(move.power * mult * variance));
      ...
      setTimeout(() => {
        ...
        setBattleLog(
          `${activePlayerPokemon.name} acertou em cheio causando ${damage} de dano!${effText}`,
        );
